...
def stopword_words():
    stopword_lst = []
    with io.open("vnese_stopword.txt", "r", encoding="utf-8") as data_file:
        for words in data_file:
            stopword_lst.append(words)
        data_file.close()
    return stopword_lst
...
class Config(object):
    ...
    def read_seeds(self, seeds_file, holder):
         with io.open(seeds_file, "r", encoding="utf-8") as data_file:
